...
以下是 `--demo` 模式的真实运行日志（3 篇内置论文摘要，所用模型由环境变量 `OPENAI_MODEL` 决定）：
...
生成的综述草稿（第 1 个小节，完整输出见 `output/review_draft.md`）：
...
- **一致性检查可能改变小节结构**：最后一步会让模型重新输出全文以统一引用编号、消除重复，个别情况下它可能合并或省略小节（实测中生成 5 节、最终输出为 4 节）。如需严格保留逐节结构，可在 `agents/synthesis_agent.py` 中跳过这一步。
